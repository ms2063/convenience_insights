...
def viz_1_1(quarter, sliced_EIBF_for_1_1):
    ...
    layout = go.Layout(
        ...
        yaxis=dict(title='예상 매출 금액'),  # y축 레이블 설정
    ...
    # 그래프 설정 변경
    fig.update_layout(
        title_font_family="NanumGothic",
        xaxis=dict(title_font=dict(family="NanumGothic", color="black"), tickfont=dict(family="NanumGothic", color='black')),
        yaxis=dict(title_font=dict(family="NanumGothic", color="black"), tickfont=dict(family="NanumGothic", color='black'))
    )
    # 그래프 출력
    return fig
...
def expectation_content():
    ...
    if viz_cat_option == '선택 상권 예상 매출':
        ...
        with tab1:
            ...
            st.write(f'{quarter_options[select_quarter]}분기 시간대별 예상매출 비교 표 :')
